...
function allPostsByYear() {
	...
	$veryallpost    = array();
	/*
	 * First we get all the WP posts, post or attachment
	 * then we make an array with post only
	 * then complete the array with attachment
	 * next we split by year
	 * finished by a krsort
	 * this code is provided by a front-end dev, so it's quite dirty, i'm pretty sure
	 */
	foreach ( $allposts as $_post ) {
		$posttype = $_post->post_type;
		if ( $posttype !== 'attachment' ) {
			$postid                               = $_post->ID;
			$veryallpost[ $postid ]['ID']         = $_post->ID;
			$veryallpost[ $postid ]['post_title'] = $_post->post_title;
			$veryallpost[ $postid ]['year']       = $_post->year;
		}
	}
	foreach ( $allposts as $_post ) {
		$posttype = $_post->post_type;
		if ( $posttype === 'attachment' ) {
			...
			$veryallpost[ $postid ]['id_attachment'] = $_post->ID;
		}
	}
	$totalposts = 0;
	foreach ( $veryallpost as $_post ) {
		$postyear                                                = $_post['year'];
		$postid                                                  = $_post['ID'];
		$veryallpostlol[ $postyear ][ $postid ]['id_attachment'] = $_post['id_attachment'];
		$veryallpostlol[ $postyear ][ $postid ]['ID']            = $postid;
		$veryallpostlol[ $postyear ][ $postid ]['post_title']    = $_post['post_title'];
		$veryallpostlol[ $postyear ][ $postid ]['year']          = $postyear;
		$totalposts ++;
	}
	krsort( $veryallpostlol );
...
//	                echo "\n" . $post_id . " " . $post_img_att;
//	                echo "\$post_img_att " . $post_img_att . "\n";
//	                print_r($p);
					$post_url = get_permalink( $post_id );

					$post_title       = $p['post_title'];
					$micromix_number_ = $totalposts;
					$micromix_number  = str_pad( $micromix_number_, 3, '0', STR_PAD_LEFT );
					$mp3_uri          = "/upload/Micromix " . $micromix_number . " - " . convertAccentsAndSpecialToNormal( trim( str_ireplace( "'", " ", $post_title ) ) ) . ".mp3";
...
//                  $fat_image_src = empty($fat_image_src) ? theme_path . '/img/steven-seagal-album-cover.jpg' : "" . $fat_image_src;
					...
					if ( is_user_logged_in() && isset( $_GET['migration'] ) ) {
						...
						if ( $mp3_uri != $oldmp3name ) {
							copy( getcwd() . $oldmp3name, getcwd() . $mp3_uri );
//							echo "\nmp3 physique  | " . $oldmp3name . "\n";
//							echo "mp3 simulé | " . $mp3_uri . "\n";
//		                    echo "\n";
						}
					}
					$totalposts --;
